...
// runCompletion handles `databricks completion <shell>`.
func runCompletion(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "Usage: databricks completion <bash|zsh|fish>")
		return 1
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "databricks completion: unknown shell %q (supported: bash zsh fish)\n", shell)
		return 1
	}
	return 0
}
...
// `complete -F` registration lines stripped) and a git-style `_databricks`
...
func generateBash() string {
	...
	b.WriteString("}\n")
	b.WriteString("complete -F _databricks databricks\n")
	return b.String()
}
...
// headers and trailing invocation lines stripped) and a `_databricks` dispatch
...
func generateZsh() string {
	var b strings.Builder
	b.WriteString("#compdef databricks\n\n")
	var available []agent
...
func generateFish() string {
	var b strings.Builder
	b.WriteString("# Shell completions for databricks (multiplexer)\n")
	b.WriteString("# Generated by: databricks completion fish\n\n")
	b.WriteString("complete -c databricks -f\n\n")
	for _, a := range agents {
		desc := strings.ReplaceAll(a.Summary, "'", "\\'")
		b.WriteString(fmt.Sprintf("complete -c databricks -n '__fish_use_subcommand' -a '%s' -d '%s'\n",
			a.Name, desc))
	}
	b.WriteString("complete -c databricks -n '__fish_use_subcommand' -a 'list' -d 'List registered agents'\n")
	b.WriteString("complete -c databricks -n '__fish_use_subcommand' -a 'completion' -d 'Emit shell completion'\n")
	return b.String()
}
